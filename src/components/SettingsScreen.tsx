...
import { Download, ExternalLink, FileUp, HardDrive, RadioTower, RefreshCw, Route, Settings2, ShieldCheck } from "lucide-react";
    ...
    <div className="screen settings-screen">
      ...
      <SimpleGrid className="settings-grid" cols={{ base: 1, lg: 2 }} spacing="lg">
        <Paper className="glass-panel settings-section" radius="xl" p="xl">
          ...
          <HelpTip label="Open General Information and copy the Application ID. Do not copy the client secret."><Button mt="md" variant="subtle" color="ultraviolet" leftSection={<ExternalLink size={15} />} onClick={() => openUrl("https://discord.com/developers/applications")}>Open Developer Portal</Button></HelpTip>
        ...
        <Paper className="glass-panel settings-section" radius="xl" p="xl">
          ...
          <Stack gap="xs">
            <Paper className="setting-toggle" p="md" radius="lg">
              <Switch label="Launch at login" description="Starts hidden in the tray. Takes effect when you save." checked={config.settings.launchAtLogin} onChange={(event) => updateSettings({ launchAtLogin: event.currentTarget.checked })} color="teal" />
            </Paper>
            <Paper className="setting-toggle" p="md" radius="lg">
              <Switch label="Close to tray" description="Closing the window keeps publishing. Quit from the tray icon to stop." checked={config.settings.closeToTray} onChange={(event) => updateSettings({ closeToTray: event.currentTarget.checked })} color="teal" />
            </Paper>
          </Stack>
          <HelpTip label="Checks GitHub for a newer installer and asks before installing. The app restarts afterward."><Button mt="md" variant="light" color="ultraviolet" leftSection={<RefreshCw size={15} />} loading={checkingUpdates} onClick={() => void onCheckForUpdates()}>Check for updates</Button></HelpTip>
        </Paper>
        ...
        <Paper className="glass-panel settings-section" radius="xl" p="xl">
          ...
          <Group grow>
            ...
            <HelpTip fill label="Write the current setup to a file. Unsaved edits are saved first."><Button variant="light" color="ultraviolet" leftSection={<Download size={16} />} onClick={exportConfiguration}>Export JSON</Button></HelpTip>
          </Group>
